...
# The native library must be locatable before `using Eunoia`. This repo has no
# Cargo workspace (the `eunoia-capi` source lives in jolars/eunoia), so the
# library cannot be built from here: either point EUNOIA_CAPI_LIB at a locally
